...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void previewCamera() {
        PreviewEngine.from(this)                                                //预览图引擎
                .setCameraRatio(AspectRatio.RATIO_4_3)                          //设置相机比例(默认4:3)
                .showFacePoints(false)                                          //是否显示人脸关键点
                .showFps(true)                                                  //是否显示FPS
                .backCamera(true)                                               //打开后置摄像头
                .setGalleryListener(new OnGallerySelectedListener() {           //图库监听器
                    @Override
                    public void onGalleryClickListener(GalleryType type) {
                        scanMedia(type == GalleryType.ALL);             //扫描图库
                    }
                })
                .setPreviewCaptureListener(new OnPreviewCaptureListener() {      //设置拍照监听器
                    @Override
